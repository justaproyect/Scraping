...
#panelCfg textarea:focus{border-color:#ff3333}
#panelCfg .row{display:flex;gap:8px;margin-bottom:10px;align-items:center}
#panelCfg .row label{min-width:100px;margin:0}
#panelCfg .btn-sv{background:#cc0000;border:none;color:#fff;padding:8px 20px;border-radius:3px;font-size:12px;cursor:pointer;font-family:inherit}
#panelCfg .btn-sv:hover{background:#990000}
#panelCfg .btn-sv:disabled{opacity:.4;cursor:default}
#panelCfg .st{color:#555;font-size:11px;margin-left:8px}
</style>
</head>
<body>
...
<div id="panelCfg">
  ...
  <div class="row" style="margin-top:8px;border-top:1px solid #222;padding-top:8px;flex-wrap:wrap">
    <label style="min-width:50px;font-size:10px">SendGrid</label><input type="password" id="sgKey" placeholder="SG.xxxxx API Key (recomendado para Render)" style="flex:3;min-width:150px;padding:6px 8px;background:#111;border:1px solid #333;border-radius:3px;color:#e0e0e0;font-size:12px;font-family:inherit;outline:none">
    <span id="smtpCfgStatus" style="color:#555;font-size:10px"></span>
  </div>
  <div class="row" style="gap:8px;flex-wrap:wrap">
    <label style="min-width:50px;font-size:10px">SMTP User</label><input type="text" id="smtpUser" placeholder="usuario@gmail.com" style="flex:2;min-width:120px;padding:6px 8px;background:#111;border:1px solid #333;border-radius:3px;color:#e0e0e0;font-size:12px;font-family:inherit;outline:none">
    <label style="min-width:40px;font-size:10px">Pass</label><input type="password" id="smtpPass" placeholder="App Password" style="flex:2;min-width:120px;padding:6px 8px;background:#111;border:1px solid #333;border-radius:3px;color:#e0e0e0;font-size:12px;font-family:inherit;outline:none">
  </div>
  <div class="row" style="margin-top:12px">
    <button class="btn-sv" id="btnGuardarCfg" onclick="guardarCfg()">Guardar y regenerar</button>
    <span class="st" id="cfgStatus"></span>
    <span class="st" style="margin-left:auto"><label style="display:inline;cursor:pointer"><input type="checkbox" onchange="document.querySelectorAll('.ad').forEach(function(e){e.style.display=this.checked?'block':'none'})"> Mostrar anuncios</label></span>
  </div>
</div>
...
<script>
function toggleCfg() {
  var p = document.getElementById("panelCfg");
  p.style.display = p.style.display === "none" ? "block" : "none";
  if (p.style.display === "block") cargarCfg();
}
function cargarCfg() {
  var r = new XMLHttpRequest();
  r.open("GET", "/api/config", true);
  r.onload = function() {
...
function guardarCfg() {
  ...
  btn.disabled = true;
  st.textContent = "Regenerando mensajes...";
  var r = new XMLHttpRequest();
  r.open("POST", "/api/config", true);
  r.setRequestHeader("Content-Type", "application/json");
  r.onload = function() {
    var d = JSON.parse(r.responseText);
    if (d.ok) { st.textContent = "Listo, recargando..."; setTimeout(function() { location.reload(); }, 1500); }
    else { st.textContent = "Error: " + (d.error || "desconocido"); btn.disabled = false; }
  };
  ...
  var sgKey = document.getElementById("sgKey").value.trim();
  if (sgKey === "***configurado***") sgKey = "";
  if (smtpUser || smtpPass || sgKey) {
